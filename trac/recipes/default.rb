...
execute "trac-environment" do
  command "trac-admin #{node[:trac][:basedir]}/environment initenv '#{node[:trac][:project_name]}' 'sqlite:db/trac.db' svn #{node[:subversion][:dir]}"
...
template "trac-ini" do
  ...
  variables(
    ...
    :trac_svn_repo => node[:subversion][:dir]
  )
end
